...
def venues():
  ...
  index = len(cities) - 1
  for d in data:
    if d["city"] + d["state"] == cities[index]["city"] + cities[index]["state"]:
      cities[index]["venues"].append(d["venues"][0])
    else:
      cities.append(d)
...
def show_venue(venue_id):
  ...
  venue_shows = Venue.query.get(venue_id)

  return render_template('pages/show_venue.html', venue=venue_shows)
...
def create_venue_submission():
  ...
  if len(venues) > 0:
    flash('An error occurred. Venue ' + data['name'] + ' exists!')
    return render_template('pages/home.html')
...
def show_artist(artist_id):
  ...
  artist_shows= Artist.query.get(artist_id)
...
def edit_artist_submission(artist_id):
  ...
  if not artist:
    flash('An error occured. Artist ' + artist_id + ' does not exist!')
...
def create_artist_submission():
  ...
  # TODO: modify data to be the data object returned from db insertion
  data = request.form
  artists = Artist.query.filter_by(name=data['name']).all()

  if len(artists) > 0:
    flash('An error occurred!. Artist ' + data.name + ' exists!')
...
def create_show_submission():
  ...
  data = request.form
  shows = Show.query.filter_by(start_time=data['start_time']).all()

  if len(shows) > 0:
    flash('An error occured. Show clashes!')
